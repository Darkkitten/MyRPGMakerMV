...
Darkkitten.Param.UseImage = toBoolean(Darkkitten.Parameters['Use Image']);
...
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  ...
  if (command === "enter_text") {
    if (args.length > 1) {
      ...
      Darkkitten.Param.UseImage = toBoolean(args[2]);
      //Check if you have the script to enable images and if true then set it.
      if (Darkkitten.Param.UseImage === true) {
        for (var i = 3, j = 4, k = 5; i < args.length, j < args.length, k < args.length; i++, j++, k++) {
      ...
      //this is when Use Image is false.
        ...
        for (var i = 3, j = 4; i < args.length, j < args.length; i++, j++) {
...
/**
 * Converts a value to it's corresponding boolean value. 
 * It accepts the following as true values:
...
Scene_Input.prototype.setLetters = function () {
  for (var i = 48; i <= 57; i++) {
...
Scene_Input.prototype.revertControls = function () {
